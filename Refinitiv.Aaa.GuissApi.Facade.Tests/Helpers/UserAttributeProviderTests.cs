...
namespace Refinitiv.Aaa.GuissApi.Facade.Tests.Helpers
{
    ...
    public class UserAttributeProviderTests
    {
        ...
        {
            Mock<IUserAttributeAccessor> testAccessor = new Mock<IUserAttributeAccessor>();           
            Dictionary<IUserAttributeAccessor, List<string>> dict = new Dictionary<IUserAttributeAccessor, List<string>>();

            testAccessor.Setup(x => x.GetUserAttributesAsync(It.IsAny<string>(), It.IsAny<IEnumerable<string>>())).ReturnsAsync(new List<UserAttributeDetails>());
            dict.Add(testAccessor.Object, new List<string>());
            accessorsHelper.Setup(x => x.GetAccessorsWithAttributes(It.IsAny<IEnumerable<string>>())).Returns(dict);
